  ...
  const loadLocation = useCallback(async () => {
    try {
      ...
      if (locationData) {
        setNewLocation({
          corrected_place_name:
            locationData.corrected_place_name ||
            locationData.original_place_name ||
            '',
                ...
                <div>
                  ...
                  <div className='relative'>
                    ...
                    {showSuggestions && suggestions.length > 0 && (
                      <div
                        className={`${glass.surfaceStrong} absolute z-10 mt-1 w-full border border-white/10 rounded-lg shadow-lg max-h-40 overflow-y-auto`}
                      >
                        {suggestions.map((suggestion, index) => (
                          <button
                            ...
                            onClick={() => {
                              setNewLocation((prev) => ({
                                ...prev,
                                corrected_place_name: suggestion,
                              }));
                              setShowSuggestions(false);
                            }}
                  ...
                  <div className='text-xs text-muted-foreground p-3 rounded-lg bg-white/5'>
                    ...
                    {location.original_place_name && (
                      <div className='flex items-center gap-2 mt-1'>
                        <Globe size={12} />
                        <span>
                          Original GPS name: {location.original_place_name}
                        </span>
                      </div>
                    )}
                  </div>
          ...
          >
            ...
            <div className='flex items-center gap-4 text-xs text-muted-foreground'>
              ...
              <div className='flex items-center gap-1'>
                <MapPin size={12} />
                <span>
                  {Math.abs(cluster.max_lat - cluster.min_lat).toFixed(4)}° ×{' '}
                  {Math.abs(cluster.max_lng - cluster.min_lng).toFixed(4)}°
                </span>
              </div>
            </div>
          </div>
