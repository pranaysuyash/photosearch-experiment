...
async def process_directory_faces(
    directory_path: str, background_tasks: BackgroundTasks, show_progress: bool = False
) -> JSONResponse:
    ...
    try:
        ...
        async def process_task():
            try:
                ...
                results = manager.face_clusterer.process_directory(directory_path, show_progress=show_progress)
                manager.update_job_status(
                    job_id,
                    "completed",
                    f"Processed {results['processed_images']} images",
                    100,
                )
...
async def label_face_cluster(request: FaceClusterLabelRequest) -> JSONResponse:
    ...
    try:
        ...
        manager.face_clusterer.label_face_cluster(request.cluster_id, request.person_name)
        ...
        async def scan_task():
            try:
                ...
                manager.update_job_status(
                    job_id,
                    "completed",
                    f"Found {results['groups_created']} duplicate groups",
                    100,
                )
                manager.active_jobs[job_id]["results"] = results
...
async def get_duplicate_groups(
    group_type: Optional[str] = None, min_similarity: Optional[float] = None
) -> JSONResponse:
...
async def get_duplicate_suggestions(group_id: str) -> JSONResponse:
    ...
    try:
        ...
        return JSONResponse({"status": "success", "group_id": group_id, "suggestions": suggestions})
        ...
        async def extract_task():
            try:
                ...
                for i, image_path in enumerate(request.image_paths):
                    ...
                    results.append({"image_path": image_path, "result": ocr_result.__dict__})

                    progress = ((i + 1) / len(request.image_paths)) * 100
                    manager.update_job_status(
                        job_id,
                        "processing",
                        f"Processed {i+1}/{len(request.image_paths)} images",
                        progress,
                    )
...
async def search_text(request: OCRSearchRequest) -> JSONResponse:
    ...
    try:
        ...
        matches = manager.ocr_search.search_text(request.query, request.language, request.min_confidence, request.limit)
...
async def get_job_status(job_id: str) -> JSONResponse:
    ...
    try:
        ...
        return JSONResponse({"status": "success", "job_id": job_id, "job_status": job_status})
