...
def test_usage_tracking():
    ...
    assert can_add == True, "Should be able to add 3000 more images (total 4000 < 5000 limit)"
    print("✓ Limit checking works for allowed addition")
    
    can_add_too_much = pricing_manager.check_limit(user_id, 5000)
    assert can_add_too_much == False, "Should not be able to add 5000 more images (total 6000 > 5000 limit)"
...
def test_tier_upgrades():
    ...
    assert success == True, "Upgrade to basic tier should succeed"
    ...
    assert invalid_success == False, "Upgrade to invalid tier should fail"
