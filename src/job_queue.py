...
class JobQueue:
    ...
    def start_workers(self):
        ...
        for i in range(self.num_workers):
            worker = threading.Thread(target=self._worker_thread, name=f"JobWorker-{i+1}", daemon=True)
            worker.start()
            self.workers.append(worker)

        logger.info(f"Started {self.num_workers} job workers")
    ...
    with JobQueue(args.db, args.workers) as job_queue:
        ...
        def test_handler(job, update_callback):
            ...
            for i in range(1, 11):
                time.sleep(0.5)
                progress = i * 10
                update_callback(progress=progress, message=f"Processing step {i}/10")

            # Return result
            result = {"message": "Test job completed", "steps": 10, "data": job.payload}
        ...
        if args.test:
            ...
            try:
                while True:
                    ...
                    print(
                        f"\rJobs: {stats['total_jobs']} | Pending: {stats['status_counts'].get('pending', 0)} | Processing: {stats['status_counts'].get('processing', 0)} | Completed: {stats['status_counts'].get('completed', 0)} | Failed: {stats['status_counts'].get('failed', 0)}",
                        end="",
                    )
