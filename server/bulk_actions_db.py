...
class BulkActionsDB:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute("""
                CREATE TABLE IF NOT EXISTS bulk_action_history (
                    ...
                    INDEX idx_bulk_action_id (bulk_action_id)
                )
            """)

    def record_bulk_action(self, 
                          action_type: str, 
                          user_id: str, 
                          affected_paths: List[str], 
                          operation_data: Optional[Dict[str, Any]] = None) -> str:
        """
    ...
    def can_undo_action(self, action_id: str) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                result = conn.execute(
                    "SELECT status FROM bulk_actions WHERE id = ?",
                    (action_id,)
                ).fetchone()
