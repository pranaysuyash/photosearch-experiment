        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute(
                    ...
                    """,
                    (location_id, photo_path, latitude, longitude, original_place_name, 
                     corrected_place_name, country, region, city, accuracy)
                )
                return location_id
    ...
    def get_location_stats(self) -> Dict[str, int]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                result = conn.execute("SELECT COUNT(DISTINCT city) as unique_cities FROM photo_locations WHERE city IS NOT NULL").fetchone()
                unique_cities = result['unique_cities'] if result else 0

                result = conn.execute("SELECT COUNT(DISTINCT region) as unique_regions FROM photo_locations WHERE region IS NOT NULL").fetchone()
                unique_regions = result['unique_regions'] if result else 0

                result = conn.execute("SELECT COUNT(DISTINCT country) as unique_countries FROM photo_locations WHERE country IS NOT NULL").fetchone()
            ...
            return {'total_locations': 0, 'unique_cities': 0, 'unique_regions': 0, 'unique_countries': 0}
