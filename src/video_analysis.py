...
try:
    ...
    logger = logging.getLogger(__name__)
    logger.warning("OCR modules not available - text detection will be disabled")
...
class VideoAnalyzer:
    ...
    def _is_video_processed(self, video_path: str) -> bool:
        ...
        try:
            cursor = conn.execute("SELECT COUNT(*) as count FROM video_metadata WHERE video_path = ?", (video_path,))
    ...
    def get_video_analysis(self, video_path: str) -> Dict[str, Any]:
        ...
        try:
            # Get metadata
            cursor = conn.execute("SELECT * FROM video_metadata WHERE video_path = ?", (video_path,))
            ...
            # Get keyframes
            cursor = conn.execute(
                "SELECT * FROM video_keyframes WHERE video_path = ? ORDER BY timestamp_seconds", (video_path,)
            )
            keyframes = [dict(row) for row in cursor.fetchall()]

            # Get scenes
            cursor = conn.execute(
                "SELECT * FROM video_scenes WHERE video_path = ? ORDER BY scene_number", (video_path,)
            )
            scenes = [dict(row) for row in cursor.fetchall()]

            # Get OCR results
            cursor = conn.execute(
                "SELECT * FROM video_ocr WHERE video_path = ? ORDER BY timestamp_seconds", (video_path,)
            )
