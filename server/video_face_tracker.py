...
class VideoFaceTracker:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(str(self.db_path)) as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_video_detections_timestamp ON video_face_detections(timestamp)")
    ...
    def track_faces_in_video(self, video_path: str, sample_rate: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            while True:
                ...
                if faces:
                    # Update tracking
                    active_tracks = self._update_tracks(
                        active_tracks, faces, frame_number, timestamp
                    )
                ...
                active_tracks = self._cleanup_lost_tracks(
                    active_tracks, frame_number, self.max_track_gap
                )
    ...
    def _calculate_face_quality(self, frame: np.ndarray, bbox: List[int]) -> Dict[str, float]:
        ...
        if face_region.size == 0:
            return {'overall': 0.0, 'size': 0.0, 'sharpness': 0.0, 'frontal': 0.0, 'lighting': 0.0}
    ...
    def _update_tracks(self, active_tracks: Dict[str, Dict], faces: List[Dict], 
                      frame_number: int, timestamp: float) -> Dict[str, Dict]:
    ...
    def _calculate_face_similarity(self, face1: Dict, face2: Dict) -> float:
        ...
        embedding_sim = 0.0
        if (face1.get('embedding') is not None and 
            face2.get('embedding') is not None):
    ...
    def _match_faces_to_tracks(self, similarities: Dict[Tuple[str, int], float], 
    ...
    def _cleanup_lost_tracks(self, active_tracks: Dict[str, Dict], 
                           current_frame: int, max_gap: int) -> Dict[str, Dict]:
