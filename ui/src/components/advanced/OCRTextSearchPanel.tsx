...
export function OCRTextSearchPanel() {
  ...
  const [activeTab, setActiveTab] = useState<'search' | 'extract' | 'regions' | 'settings'>('search');
  ...
  const searchText = useCallback(async (query: string, language?: string) => {
    try {
      setLoading(true);
  ...
  const extractTextBatch = useCallback(async (imagePaths: string[]) => {
    try {
      setLoading(true);
  ...
  const getTextRegions = useCallback(async (imagePath: string) => {
    try {
      const response = await api.get(`/api/ocr/regions/${encodeURIComponent(imagePath)}`);
  ...
  const handleDirectorySelect = () => {
    ...
    input.onchange = (e) => {
      ...
      if (files && files.length > 0) {
        const imagePaths = Array.from(files).map(file => file.webkitRelativePath);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-white mb-2">OCR Text Search</h2>
          <p className="text-gray-400">Search text within images using advanced OCR technology</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="text-right">
            <p className="text-white font-semibold">{stats.total_text_regions_in_db || 0}</p>
      ...
      {Object.keys(processingJobs).length > 0 && (
        <div className={`${glass.card} p-4 space-y-2`}>
          ...
          {Object.entries(processingJobs).map(([jobId, job]) => (
            <div key={jobId} className="flex items-center justify-between p-2 bg-black/20 rounded">
              <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                <span className="text-gray-400 text-xs">{job.progress.toFixed(1)}%</span>
      ...
      <AnimatePresence mode="wait">
        ...
        {activeTab === 'search' && (
          ...
          >
            ...
            <div className={`${glass.card} p-6`}>
              <h3 className="text-xl font-semibold text-white mb-4">Search Text in Images</h3>
              ...
              <div className="flex items-center gap-4 text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <select
                    value=""
                    onChange={(e) => e.target.value && searchText(searchQuery, e.target.value)}
                    className="px-2 py-1 bg-black/30 border border-white/20 rounded text-white text-sm"
                  >
                    <option value="">All Languages</option>
                    {SUPPORTED_LANGUAGES.filter(lang => selectedLanguages.includes(lang.code)).map(lang => (
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    className="w-24"
                  />
                  <span className="text-gray-400 text-xs w-8">{(minConfidence * 100).toFixed(0)}%</span>
                </div>

                <button
                  onClick={() => setShowHighlighting(!showHighlighting)}
                  className={`flex items-center gap-1 px-2 py-1 rounded ${showHighlighting ? 'bg-blue-600 text-white' : 'bg-white/10 text-gray-400'}`}
                >
                  ...
                  {searchResults.map((region, index) => (
                    ...
                    >
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex items-center gap-3">
                          <span className="text-lg">{getLanguageFlag(region.language_code)}</span>
                          <span className="text-white font-medium capitalize">
                            {SUPPORTED_LANGUAGES.find(l => l.code === region.language_code)?.name || region.language_code}
                          </span>
                          <span className={`text-sm ${getConfidenceColor(region.confidence_score)}`}>
                      ...
                      <div className="mb-3">
                        {renderHighlightedText(region.text_content, region.highlighted_text)}
                      </div>

                      <div className="flex items-center justify-between">
                        <span className="text-gray-400 text-sm">{region.photo_path.split('/').pop()}</span>
                        <div className="flex items-center gap-2">
        ...
        {activeTab === 'extract' && (
          ...
          >
            <div className={`${glass.card} p-6`}>
              <h3 className="text-xl font-semibold text-white mb-4">Text Extraction</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  ...
                  <div className="space-y-2">
                    {SUPPORTED_LANGUAGES.map(lang => (
                      ...
                      >
                        ...
                        <span className={`flex-1 ${lang.supported ? 'text-white' : 'text-gray-500'}`}>
                          {lang.name}
                        </span>
                        {!lang.supported && (
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <h5 className="text-white font-medium">Handwriting Recognition</h5>
                        <p className="text-gray-400 text-sm">Extract handwritten text using EasyOCR</p>
                      </div>
                      <button
                        onClick={() => setEnableHandwriting(!enableHandwriting)}
                        className={`w-12 h-6 ${enableHandwriting ? 'bg-blue-600' : 'bg-gray-600'} rounded-full relative`}
                      >
                        <div className={`absolute ${enableHandwriting ? 'right-1' : 'left-1'} top-1 w-4 h-4 bg-white rounded-full transition-transform`} />
                      </button>
                    </div>

                    <div className="flex items-center justify-between">
                      <div>
                        <h5 className="text-white font-medium">Auto Language Detection</h5>
                        <p className="text-gray-400 text-sm">Automatically detect text languages</p>
                      </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <h5 className="text-white font-medium">Confidence Threshold</h5>
                        <p className="text-gray-400 text-sm">Minimum confidence for text extraction</p>
                      </div>
                      <input
                        type="range"
                        min="0"
              ...
              <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="text-blue-400 font-medium mb-1">Extraction Information</h4>
                    <p className="text-gray-300 text-sm">
                      Text extraction uses Tesseract OCR for printed text and EasyOCR for handwritten text.
                      Processing time varies based on image complexity and selected languages.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className={`${glass.card} p-6`}>
              <h3 className="text-xl font-semibold text-white mb-4">Directory Processing</h3>
        ...
        {activeTab === 'regions' && (
          ...
          >
            <div className={`${glass.card} p-6`}>
              <h3 className="text-xl font-semibold text-white mb-4">Text Regions</h3>

              {selectedRegions.length > 0 ? (
                <div className="space-y-4">
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div>
                      <h4 className="text-white font-medium mb-3">Image with Text Regions</h4>
                      <div className="relative bg-black/20 rounded-lg overflow-hidden">
                        <img
                          src={`/api/image/${encodeURIComponent(selectedRegions[0].photo_path)}`}
                          alt="Text regions"
                    ...
                    <div>
                      <h4 className="text-white font-medium mb-3">Extracted Text</h4>
                      <div className="space-y-3 max-h-96 overflow-y-auto">
                        {selectedRegions.map((region, index) => (
                          <div key={region.id} className="p-3 bg-black/20 rounded-lg">
                            <div className="flex items-center justify-between mb-2">
                              <span className="text-white font-medium">Region {index + 1}</span>
                              <div className="flex items-center gap-2">
                                <span className={`text-sm ${getConfidenceColor(region.confidence_score)}`}>
                                  {(region.confidence_score * 100).toFixed(1)}%
                                </span>
                            ...
                            <p className="text-gray-300 text-sm mb-2">{region.text_content}</p>
                            <div className="flex items-center gap-4 text-xs text-gray-500">
                              <span>Position: ({region.bbox.x}, {region.bbox.y})</span>
                              <span>Size: {region.bbox.width}×{region.bbox.height}</span>
                              <span className="capitalize">{region.text_type}</span>
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  <div className="pt-4 border-t border-white/10">
                    ...
                    <div className="p-4 bg-black/20 rounded-lg">
                      <p className="text-gray-300 whitespace-pre-wrap">
                        {selectedRegions.map(region => region.text_content).join('\n')}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-400">Search for text or extract from images to see text regions</p>
                </div>
              )}
            </div>
          </motion.div>
        ...
        {activeTab === 'settings' && (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <h4 className="text-white font-medium mb-4">Processing Preferences</h4>
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <h5 className="text-white font-medium">GPU Acceleration</h5>
                      <p className="text-gray-400 text-sm">Use GPU for faster text processing</p>
                    </div>
                    <button className="w-12 h-6 bg-blue-600 rounded-full relative">
                      <div className="absolute right-1 top-1 w-4 h-4 bg-white rounded-full" />
                    </button>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-gray-400 text-sm">Process multiple images simultaneously</p>
                    </div>
                    <button className="w-12 h-6 bg-blue-600 rounded-full relative">
                      <div className="absolute right-1 top-1 w-4 h-4 bg-white rounded-full" />
                    </button>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <h5 className="text-white font-medium">Auto-enhance Images</h5>
                      <p className="text-gray-400 text-sm">Improve image quality before OCR</p>
                    </div>
                    <button className="w-12 h-6 bg-blue-600 rounded-full relative">
                      <div className="absolute right-1 top-1 w-4 h-4 bg-white rounded-full" />
                    </button>
              ...
              <div>
                <h4 className="text-white font-medium mb-4">Quality & Performance</h4>
                <div className="space-y-4">
                  <div>
                    <label className="block text-gray-300 text-sm mb-2">Processing Threads</label>
                  ...
                  <div>
                    <label className="block text-gray-300 text-sm mb-2">Image Quality Threshold</label>
                    <input
                      type="range"
                      min="0"
                      max="100"
                  ...
                  <div>
                    <label className="block text-gray-300 text-sm mb-2">Cache Size (MB)</label>
                    <input
                      type="number"
                      defaultValue="512"
