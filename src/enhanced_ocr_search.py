...
import logging
import hashlib
from typing import List, Dict, Optional, Any, Callable, Set, cast
...
try:
    ...
    logger.warning(
        "pytesseract not available. Install with: pip install pytesseract and ensure Tesseract OCR is installed on the system"
    )
...
class EnhancedOCRSearch:
    ...
    def _is_test_mode(self) -> bool:
        ...
        We treat OCR model downloads / heavyweight initialization as opt-in in
        tests to keep `python -m pytest` fast and deterministic.
        """

        if os.getenv("PHOTOSEARCH_TEST_MODE") == "1":
            return True
    ...
    def _download_missing_models(self):
        ...
        try:
            ...
            if missing_langs:
                ...
                logger.info(
                    "Install with: apt-get install tesseract-ocr-<lang> (Linux) or brew install tesseract-lang (Mac)"
                )
    ...
    def extract_text_batch(
                ...
                try:
                    ...
                    if show_progress and (i + 1) % 10 == 0:
                        ...
                        if self.progress_callback:
                            self.progress_callback(
                                f"Progress: {progress:.1f}% - {results['text_regions_found']} text regions"
                            )
        ...
        if show_progress and self.progress_callback:
            self.progress_callback(
                f"Completed: {results['processed_images']} images, {results['text_regions_found']} text regions"
            )
    ...
    def _highlight_text(self, text: str, query: str) -> str:
        ...
        for word in query_words:
            ...
            while start != -1:
                ...
                highlighted = (
                    highlighted[:insert_pos]
                    + f"<mark>{highlighted[insert_pos:insert_pos+len(word)]}</mark>"
                    + highlighted[insert_pos + len(word) :]
                )
