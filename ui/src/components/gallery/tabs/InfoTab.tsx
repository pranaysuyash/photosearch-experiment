...
export function InfoTab({
    ...
    const fileDate = metadata?.filesystem?.created?.split('T')[0] || 'Unknown date';
    ...
    return (
        <div className="flex flex-col gap-6 h-full text-white/90">
            ...
            <div className="flex flex-wrap items-center gap-2 p-3 bg-white/5 rounded-2xl border border-white/10 backdrop-blur-md">
                ...
                />
                <ControlButton onClick={onAddToAlbum} icon={FolderPlus} label="Add to Album" />
                <ControlButton onClick={onEdit} icon={Edit3} label="Edit Photo" highlight />
                ...
                <ControlButton onClick={() => onFlip('horizontal')} icon={FlipHorizontal} label="Flip Horizontal" />
                <ControlButton onClick={() => onFlip('vertical')} icon={FlipVertical} label="Flip Vertical" />
            ...
            <div className="glass-surface p-4 rounded-xl">
                <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-bold uppercase tracking-wider text-white/50">Rating</span>
                </div>
                <StarRating
...
interface ControlButtonProps {
    onClick: () => void;
    icon: any; // Using any to bypass strict React.ElementType issues with Lucide icons
    label: string;
    active?: boolean;
    activeColor?: string;
    highlight?: boolean;
...
function ControlButton({ onClick, icon: Icon, label, active, activeColor, highlight, variant = 'default' }: ControlButtonProps) {
    const baseClass = "p-2 rounded-lg transition-all duration-200 flex items-center justify-center group relative";
