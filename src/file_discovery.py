...
ANIMATED_FORMATS = {".gif", ".gifv", ".apng"}
...
def scan_directories(
    start_path: Optional[str] = None, incremental: bool = False, existing_catalog: Optional[Dict] = None
) -> Dict:
    """
        ...
        try:
            ...
            for filename in files:
                ...
                try:
                    ...
                    file_info = {"name": filename, "type": media_type, "size": stat.st_size, "mtime": stat.st_mtime}
...
def detect_changes(old_catalog: Dict, new_scan: Dict) -> Dict:
    ...
    """
    changes: Dict[str, List[str]] = {"files_added": [], "files_removed": [], "folders_added": [], "folders_removed": []}
...
Examples:
  ...
  python file_discovery.py --search-file vacation.jpg
    ...
    parser.add_argument(
        "--catalog", type=str, default="media_catalog.json", help="Catalog file path (default: media_catalog.json)"
    )
    ...
    if not any([args.scan, args.incremental, args.search_folder, args.search_file, args.stats, args.changes]):
    ...
    if args.scan:
        ...
        catalog = {
            "metadata": {
                ...
                "total_images": sum(1 for files in scan_result.values() for f in files if f["type"] == "image"),
                "total_videos": sum(1 for files in scan_result.values() for f in files if f["type"] == "video"),
                "total_animated": sum(1 for files in scan_result.values() for f in files if f["type"] == "animated"),
        ...
        if choice == "1":
            ...
            catalog = {
                "metadata": {
                    ...
                    "total_videos": sum(1 for files in scan_result.values() for f in files if f["type"] == "video"),
                    "total_animated": sum(
                        1 for files in scan_result.values() for f in files if f["type"] == "animated"
                    ),
                    "last_changes": {"files_added": 0, "files_removed": 0, "folders_added": 0, "folders_removed": 0},
                },
                "catalog": scan_result,
