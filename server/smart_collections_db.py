...
class SmartCollectionsDB:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                    """,
                    (collection_id, name, description, json.dumps(rule_definition), 
                     auto_update, privacy_level)
                )
    ...
    def get_smart_collection(self, collection_id: str) -> Optional[SmartCollection]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                result = conn.execute(
                    "SELECT * FROM smart_collections WHERE id = ?",
                    (collection_id,)
                ).fetchone()
    ...
    def update_smart_collection(self, 
                               collection_id: str, 
    ...
    def get_photos_for_collection(self, collection_id: str) -> List[str]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.row_factory = sqlite3.Row
                result = conn.execute(
                    "SELECT rule_definition FROM smart_collections WHERE id = ?",
                    (collection_id,)
                ).fetchone()
                ...
                rule_definition = json.loads(result['rule_definition'])
    ...
    def get_collections_stats(self) -> Dict[str, int]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                result = conn.execute("SELECT COUNT(*) as auto_update FROM smart_collections WHERE auto_update = 1").fetchone()
