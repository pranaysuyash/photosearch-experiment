...
export function ClusterManagement({
  ...
  const [qualityAnalysis, setQualityAnalysis] =
    useState<ClusterQualityResult | null>(null);
  ...
  const displayedClusters = showHidden
    ? [...clusters, ...hiddenClusters]
    : clusters;
          ...
          <div className='flex items-center gap-3'>
            ...
            <button
              ...
              className={`btn-glass px-3 py-2 flex items-center gap-1 ${
                showHidden ? 'btn-glass--primary' : 'btn-glass--muted'
              }`}
              ...
              <button
                onClick={() => {
                  const cluster = async () => {
                    try {
                      setLoading(true);
                      const result: ClusteringResult = await api.clusterFaces(
                        0.6,
                        2
                      );
                      if (result.success) {
            ...
            {!loading && displayedClusters.length > 0 && (
              <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6'>
                {displayedClusters.map((cluster) => {
                  const isHidden = hiddenClusters.some(
                    (h) => h.cluster_id === cluster.cluster_id
                  );
                  ...
                  return (
                    <div
                      ...
                      className={`${
                        glass.surface
                      } p-4 rounded-lg cursor-pointer hover:border-blue-400 transition-all ${
                    ...
                    >
                      ...
                      <div className='grid grid-cols-2 gap-2 mb-3 text-sm'>
                        ...
                        <div>
                          ...
                          <div className='font-medium text-foreground'>
                            {cluster.photo_count || cluster.face_count}
                          </div>
                        </div>
                      </div>

                      {/* Quality indicator */}
                      {qualityAnalysis?.quality_analysis?.cluster_id ===
                        cluster.cluster_id && (
            ...
            {selectedCluster && qualityAnalysis && (
              <div className={`${glass.surfaceStrong} mt-8 p-6 rounded-lg`}>
                <div className='flex items-center justify-between mb-4'>
                  <h2 className='text-xl font-bold text-foreground'>
                    {selectedCluster.label ||
                      `Person ${selectedCluster.cluster_id.slice(-4)}`}
                  </h2>
                  <button
                ...
                <div className='grid grid-cols-2 md:grid-cols-4 gap-4 mb-6'>
                  ...
                  <div className='text-center p-3 bg-white/5 rounded-lg'>
                    <div className='text-2xl font-bold text-foreground'>
                      {qualityAnalysis.quality_analysis.avg_confidence.toFixed(
                        2
                      )}
                    </div>
                    <div className='text-xs text-muted-foreground mt-1'>
                  ...
                  <div className='text-center p-3 bg-white/5 rounded-lg'>
                    <div className='text-2xl font-bold text-foreground'>
                      {qualityAnalysis.quality_analysis.coherence_score.toFixed(
                        2
                      )}
                    </div>
                    <div className='text-xs text-muted-foreground mt-1'>
                ...
                <div className='mb-4'>
                  ...
                  <div className='flex items-center gap-2'>
                    ...
                    <div className='text-sm text-muted-foreground'>
                      {(
                        qualityAnalysis.quality_analysis.avg_quality_score * 100
                      ).toFixed(0)}
                      % quality score
                    </div>
                  </div>
                ...
                {mergeTarget && (
                  <div className='p-4 bg-white/5 rounded-lg mb-4'>
                    ...
                    <button
                      ...
                      disabled={merging}
                      className={`btn-glass ${
                        merging ? 'btn-glass--muted' : 'btn-glass--primary'
                      } w-full mt-2`}
                    >
                      {merging ? (
                        <>
          ...
          <div>
            <div className='mb-4 p-4 bg-orange-500/10 border border-orange-500/20 rounded-lg'>
              ...
              <p className='text-sm text-muted-foreground mt-1'>
                These faces haven't been assigned to any person yet. Click on a
                face to assign it to an existing person or create a new one.
              </p>
            </div>
            ...
            {unknownFaces.length === 0 ? (
              <div className='text-center py-20'>
                ...
                <p className='text-muted-foreground'>
                  No unidentified faces. All detected faces have been assigned
                  to people.
                </p>
              </div>
            ) : (
              <div className='grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3'>
                {unknownFaces.map((face) => (
                  <div
                    ...
                    onClick={() => {
                      // Open face assignment dialog
                      // TODO: Implement face assignment modal
                    }}
                  >
