...
export function DuplicateManagementPanel() {
  ...
  const [activeTab, setActiveTab] = useState<'groups' | 'scan' | 'suggestions' | 'analytics'>('groups');
  ...
  const loadDuplicateGroups = useCallback(async () => {
    try {
      ...
      const totalSaved = response.data.groups.reduce((sum: number, group: DuplicateGroup) => {
  ...
  const startDuplicateScan = useCallback(async (directoryPath: string, similarityThreshold: number = 5.0) => {
    try {
      setLoading(true);
      setScanProgress(0);

      const response = await api.scanDuplicatesWithPath(directoryPath, similarityThreshold);
  ...
  const resolveDuplicate = useCallback(async (groupId: string, action: string, targetPhotos: string[]) => {
    try {
  ...
  const getGroupTypeColor = (type: string) => {
    switch (type) {
      case 'exact': return 'text-red-500';
      case 'near': return 'text-orange-500';
  ...
  const getGroupTypeIcon = (type: string) => {
    switch (type) {
      case 'exact': return Copy;
      case 'near': return Layers;
      case 'similar': return Image as ImageIcon;
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-white mb-2">Duplicate Management</h2>
          <p className="text-gray-400">Find and manage duplicate images to save storage space</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="text-right">
            <p className="text-white font-semibold">{formatFileSize(totalSpaceSaved)}</p>
      ...
      {loading && (
        <div className={`${glass.card} p-4`}>
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-gray-400 text-sm">{scanProgress.toFixed(1)}%</span>
          </div>
      ...
      <AnimatePresence mode="wait">
        ...
        {activeTab === 'groups' && (
          ...
          >
            ...
            <div className="lg:col-span-2 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-xl font-semibold text-white">Duplicate Groups</h3>
                <span className="text-gray-400 text-sm">{duplicateGroups.length} groups</span>
              </div>
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-2">
                        {React.createElement(getGroupTypeIcon(group.group_type), {
                          className: `w-4 h-4 ${getGroupTypeColor(group.group_type)}`
                        })}
                        <span className={`capitalize ${getGroupTypeColor(group.group_type)} font-medium`}>
                          {group.group_type}
                        </span>
                      </div>
                    ...
                    <div className="grid grid-cols-3 gap-1 mb-3">
                      {group.photos.slice(0, 6).map((photo, index) => (
                        <div key={index} className="aspect-square bg-black/20 rounded overflow-hidden">
                          <img
                      ...
                      {group.photos.length > 6 && (
                        <div className="aspect-square bg-black/20 rounded flex items-center justify-center">
                          <span className="text-gray-400 text-xs">+{group.photos.length - 6}</span>
                        </div>
                      )}
                    </div>

                    <div className="flex items-center justify-between text-sm">
                      <span className="text-gray-400">{formatFileSize(group.total_size_mb)}</span>
            ...
            <div className="space-y-4">
              {selectedGroup ? (
                <>
                  <div className={`${glass.card} p-4`}>
                    <h4 className="text-white font-semibold mb-3 flex items-center gap-2">
                      {React.createElement(getGroupTypeIcon(selectedGroup.group_type), {
                        className: `w-4 h-4 ${getGroupTypeColor(selectedGroup.group_type)}`
                      })}
                      {selectedGroup.group_type.charAt(0).toUpperCase() + selectedGroup.group_type.slice(1)} Duplicates
                    </h4>

                    <div className="space-y-2 mb-4">
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-400">Photos:</span>
                        <span className="text-white">{selectedGroup.photos.length}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-400">Total Size:</span>
                        <span className="text-white">{formatFileSize(selectedGroup.total_size_mb)}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-400">Similarity:</span>
                        <span className="text-white">{selectedGroup.similarity_threshold}</span>
                      </div>
                    </div>
                      ...
                      <div className="space-y-2">
                        {resolutionSuggestions.map((suggestion, index) => (
                          <div key={index} className="p-2 bg-black/20 rounded-lg">
                            ...
                            <div className="flex items-center justify-between">
                              <p className="text-green-400 text-xs">{suggestion.reasoning}</p>
                              <span className="text-gray-400 text-xs">{formatFileSize(suggestion.space_saved_mb)}</span>
                            </div>
                            <button
                              onClick={() => {
                                const targetPhotos = suggestion.target_photo
                                  ...
                                  : [];
                                resolveDuplicate(selectedGroup.id, suggestion.action, targetPhotos);
                              }}
                  ...
                  <div className={`${glass.card} p-4`}>
                    ...
                    <div className="space-y-2">
                      <button
                        onClick={() => resolveDuplicate(selectedGroup.id, 'keep_best', selectedGroup.photos.map(p => p.path))}
                        className="w-full px-3 py-2 bg-green-600/20 text-green-400 rounded-lg hover:bg-green-600/30"
                      >
                        Keep Best Quality
                      </button>
                      <button
                        onClick={() => resolveDuplicate(selectedGroup.id, 'keep_largest', selectedGroup.photos.map(p => p.path))}
                        className="w-full px-3 py-2 bg-blue-600/20 text-blue-400 rounded-lg hover:bg-blue-600/30"
                      >
                        Keep Largest Files
                      </button>
                      <button
                        onClick={() => resolveDuplicate(selectedGroup.id, 'move_edited', selectedGroup.photos.map(p => p.path))}
                ...
                <div className={`${glass.card} p-6 text-center`}>
                  ...
                  <p className="text-gray-400">Select a duplicate group to view details and actions</p>
                </div>
              )}
            </div>
          </motion.div>
        ...
        {activeTab === 'scan' && (
          ...
          >
            <div className={`${glass.card} p-6`}>
              <h3 className="text-xl font-semibold text-white mb-4">Configure Duplicate Scan</h3>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-gray-300 text-sm mb-2">Directory to Scan</label>
                  <button
                    onClick={handleDirectorySelect}
                    disabled={loading}
                ...
                <div>
                  ...
                  <select className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:border-blue-500">
                    ...
                    <option value="2" selected>Near Duplicates (5% difference)</option>
                    <option value="5">Similar Images (15% difference)</option>
                    <option value="10">Visually Similar (30% difference)</option>
                  </select>
                </div>
              </div>
              ...
              <div className="mt-6 p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="text-blue-400 font-medium mb-1">Scan Information</h4>
                    <p className="text-gray-300 text-sm">
                      The scan will compare images using multiple algorithms including MD5 hashing,
                      perceptual hashing (PHash), and color histogram analysis. This process
            ...
            <div className={`${glass.card} p-6`}>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="text-white font-medium">Include Subdirectories</h4>
                    <p className="text-gray-400 text-sm">Scan all subdirectories recursively</p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="text-white font-medium">Ignore Small Files</h4>
                    <p className="text-gray-400 text-sm">Skip files smaller than 50KB</p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="text-white font-medium">Parallel Processing</h4>
                    <p className="text-gray-400 text-sm">Use multiple CPU cores for faster scanning</p>
                  </div>
                  <button className="w-12 h-6 bg-blue-600 rounded-full relative">
                    <div className="absolute right-1 top-1 w-4 h-4 bg-white rounded-full" />
                  </button>
        ...
        {activeTab === 'suggestions' && (
          ...
          >
            ...
            <p className="text-gray-400 mb-6">
              Get smart suggestions for managing duplicates based on image quality,
              metadata, and common user patterns.
            </p>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div className="p-4 bg-black/20 rounded-lg">
                <Sparkles className="w-8 h-8 text-yellow-500 mb-3" />
                <h4 className="text-white font-medium mb-2">Quality-Based Resolution</h4>
                <p className="text-gray-400 text-sm">
                  Automatically keep the highest quality image in each duplicate group
                </p>
              </div>

              <div className="p-4 bg-black/20 rounded-lg">
                <HardDrive className="w-8 h-8 text-blue-500 mb-3" />
                <h4 className="text-white font-medium mb-2">Storage Optimization</h4>
                <p className="text-gray-400 text-sm">
        ...
        {activeTab === 'analytics' && (
          <motion.div
            ...
            className={`${glass.card} p-6`}
          >
            <h3 className="text-xl font-semibold text-white mb-4">Duplicate Analytics</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              <div className="p-4 bg-black/20 rounded-lg">
                ...
                <p className="text-2xl font-bold text-white">{duplicateGroups.length}</p>
              ...
              <div className="p-4 bg-black/20 rounded-lg">
                ...
                <p className="text-2xl font-bold text-white">
                  {duplicateGroups.reduce((sum, group) => sum + group.photos.length, 0)}
                </p>
              ...
              <div className="p-4 bg-black/20 rounded-lg">
                ...
                <p className="text-2xl font-bold text-white">{formatFileSize(totalSpaceSaved)}</p>
              ...
              <div className="p-4 bg-black/20 rounded-lg">
                ...
                <p className="text-2xl font-bold text-white">
                  {duplicateGroups.length > 0
                    ? (duplicateGroups.reduce((sum, group) => sum + group.photos.length, 0) / duplicateGroups.length).toFixed(1)
                    : '0'}
                </p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div>
                <h4 className="text-white font-medium mb-3">Duplicate Types Distribution</h4>
                <div className="space-y-2">
                  {['exact', 'near', 'similar'].map((type) => {
                    const count = duplicateGroups.filter(g => g.group_type === type).length;
                    const percentage = duplicateGroups.length > 0 ? (count / duplicateGroups.length) * 100 : 0;
                    return (
                      <div key={type} className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <div className="w-24 h-2 bg-black/30 rounded-full overflow-hidden">
                            <div
                              className={`h-full ${
                                type === 'exact' ? 'bg-red-500' :
                                type === 'near' ? 'bg-orange-500' : 'bg-yellow-500'
                              }`}
              ...
              <div>
                <h4 className="text-white font-medium mb-3">Space Savings by Type</h4>
                <div className="space-y-2">
                  {['exact', 'near', 'similar'].map((type) => {
                    const spaceSaved = duplicateGroups
                      .filter(g => g.group_type === type)
                      .reduce((sum, group) => sum + group.total_size_mb * 0.7, 0);
                    return (
                      <div key={type} className="flex items-center justify-between">
                        <span className="text-gray-300 capitalize">{type}</span>
                        <span className="text-green-400">{formatFileSize(spaceSaved)}</span>
                      </div>
                    );
                  })}
                </div>
          ...
          >
            ...
            >
              <div className="p-6 border-b border-white/10">
                <div className="flex items-center justify-between">
                  <h3 className="text-xl font-semibold text-white">Compare Duplicate Images</h3>
                  <button
                    onClick={() => setShowComparison(false)}
              ...
              <div className="p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex-1 mx-4 text-center">
                    <p className="text-gray-400 text-sm">Image {currentImageIndex + 1} of {selectedGroup.photos.length}</p>
                  </div>

                  <button
                    onClick={() => setCurrentImageIndex(Math.min(selectedGroup.photos.length - 1, currentImageIndex + 1))}
                    disabled={currentImageIndex === selectedGroup.photos.length - 1}
                    className="p-2 bg-white/10 rounded-lg hover:bg-white/20 disabled:opacity-50"
                  >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {currentImageIndex > 0 && (
                    <div>
                      <h4 className="text-white font-medium mb-2">Previous Image</h4>
                      <div className="aspect-square bg-black/20 rounded-lg overflow-hidden">
                        <img
                          src={`/api/image/${encodeURIComponent(selectedGroup.photos[currentImageIndex - 1].path)}`}
                          alt="Previous"
                  ...
                  <div>
                    <h4 className="text-white font-medium mb-2">Current Image</h4>
                    <div className="aspect-square bg-black/20 rounded-lg overflow-hidden">
                      <img
                        src={`/api/image/${encodeURIComponent(selectedGroup.photos[currentImageIndex].path)}`}
                        alt="Current"
                        className="w-full h-full object-contain"
                      />
                    </div>
