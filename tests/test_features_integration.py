...
class TestFeatureIntegrations:
    ...
    def test_location_clustering_with_photo_tags(self, client):
        ...
        photos_data = [
            ...
            ("/test/cluster2.jpg", 40.7228, -74.0160, "New York"),  # Close by, should cluster
            ("/test/outside.jpg", 34.0522, -118.2437, "Los Angeles")  # Far away, different cluster
        ]
        
        for path, lat, lng, place_name in photos_data:
            # Add location data for each photo
            client.post(f"/locations/{path}", json={
                "latitude": lat,
                "longitude": lng,
    ...
    def test_version_stack_integration_with_notes(self, client):
        ...
        version_response = client.post("/versions", json={
    ...
    def test_people_integration_with_collaborative_spaces(self, client):
        ...
        space_response = client.post("/collaborative/spaces", json={
        ...
        add_photo_response = client.post(f"/collaborative/spaces/{space_id}/photos", json={
            "photo_path": photo_path,
            "caption": "Family at gathering"
        })
        assert add_photo_response.status_code == 200
        
        # Tag people in the photo
        people_tag_response = client.post(f"/people/{photo_path}", json={
    ...
    def test_multi_tag_filtering_with_location(self, client):
        ...
        client.post(f"/locations/{photo_with_tags_and_location}", json={
            "latitude": 40.7128,
            "longitude": -74.0060,
    ...
    def test_encryption_integration_with_sharing(self, client):
        ...
        privacy_response = client.post(f"/privacy/control/{photo_path}", json={
            "owner_id": "test_user",
            "visibility": "private",
        ...
        share_response = client.post("/share", json={
            "paths": [photo_path],
            "expiration_hours": 24
        })
        ...
        assert share_response.status_code in [200, 403]  # May be restricted based on privacy
    ...
    def test_bulk_actions_with_various_photo_states(self, client):
        ...
        client.post(f"/locations/{test_photos[2]}", json={
            "latitude": 40.7128,
            "longitude": -74.0060,
        ...
        bulk_tag_response = client.post("/tags/add", json={
            "paths": test_photos,
            "tags": ["bulk_action_test"]
        })
    ...
    def test_smart_collections_with_filtered_photos(self, client):
        ...
        client.post(f"/locations/{photo_path}", json={
        ...
        client.post("/tags/add", json={
            "paths": [photo_path],
            "tags": ["europe", "vacation", "trip"]
        })
        
        client.post(f"/notes/{photo_path}", json={
            "note": "Beautiful photo from our Paris trip"
        })
        
        # Create a smart collection based on location
        collection_response = client.post("/smart-collections", json={
            "name": "European Trips",
    ...
    def test_performance_with_combined_features(self, client):
        ...
        for i, photo_path in enumerate(test_photos):
            ...
            # Add note
            client.post(f"/notes/{photo_path}", json={
                "note": f"Performance test note for photo {i}"
            })
...
class TestAPIEndpointIntegration:
    ...
    def test_version_stack_endpoints_integration(self, client):
        ...
        client.post("/photos/metadata", json={
            "path": original_path,
            "metadata": {"camera": "test_camera"}
        })
    ...
    def test_tag_filtering_endpoints_integration(self, client):
        ...
        by_tag_response = client.get(f"/tags/photos/beach")
        assert by_tag_response.status_code == 200
        
        # Test getting photos by multiple tags with AND logic
        multi_tag_response = client.post("/tags/multi-filter", json={
    ...
    def test_location_clustering_endpoints_integration(self, client):
        ...
        cluster_response = client.post("/locations/clusterize", json={
            "min_photos": 2,
