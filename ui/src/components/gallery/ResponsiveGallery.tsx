...
export function ResponsiveGallery({ 
  ...
  useEffect(() => {
    if (photos.length === 0) {
      // This would ideally come from a props or context in a real app
      setIsLoading(false);
    } else {
      setIsLoading(false);
    }
  ...
  if (error) {
    return (
      <div className={`${glass.surfaceStrong} rounded-xl border border-white/10 p-8 text-center`}>
        ...
        <h3 className="font-medium text-foreground mb-1">Error Loading Gallery</h3>
  ...
  return (
    <div className="w-full">
      {/* Toolbar */}
      <div className={`${glass.surface} rounded-t-xl border border-white/10 p-3 flex items-center justify-between`}>
            ...
            {photos.map((photo, index) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="font-medium text-foreground truncate">{photo.filename}</div>
      ...
      {selectedPhoto && (
        ...
        >
          ...
          >
            ...
            {/* Photo info */}
            <div className={`${glass.surfaceStrong} border border-white/10 rounded-xl p-4 mt-4 max-w-2xl mx-auto`}>
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="font-semibold text-foreground text-lg">{selectedPhoto.filename}</h3>
                  <p className="text-sm text-muted-foreground">
                    {selectedPhoto.metadata?.image?.width}×{selectedPhoto.metadata?.image?.height}
                  </p>
                </div>
