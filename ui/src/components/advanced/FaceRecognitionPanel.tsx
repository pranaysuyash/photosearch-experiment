...
export function FaceRecognitionPanel() {
  ...
  const [activeTab, setActiveTab] = useState<'clusters' | 'scan' | 'settings' | 'privacy'>('clusters');
  ...
  const startDirectoryProcessing = useCallback(async (directoryPath: string) => {
    try {
      setLoading(true);
      const response = await api.post('/api/face/process-directory', {
  ...
  const labelCluster = useCallback(async (clusterId: string, personName: string, privacyLevel: string = 'standard') => {
    try {
      ...
      setClusters(prev => prev.map(cluster =>
        cluster.id === clusterId
          ? { ...cluster, cluster_label: personName, privacy_level }
          : cluster
      ));
  ...
  const searchByPerson = useCallback(async (personName: string) => {
    try {
      const response = await api.get(`/api/face/search/${encodeURIComponent(personName)}`);
  ...
  const loadClusterFaces = useCallback(async (clusterId: string) => {
    try {
  ...
  const getPrivacyColor = (level: string) => {
    switch (level) {
      ...
      case 'sensitive': return 'text-yellow-500';
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400">Organize photos by people with smart face detection</p>
        </div>
        <button
      ...
      <AnimatePresence mode="wait">
        ...
        {activeTab === 'clusters' && (
          ...
          >
            ...
            <div className="lg:col-span-2 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-xl font-semibold text-white">Face Clusters</h3>
                <span className="text-gray-400 text-sm">{clusters.length} groups</span>
              </div>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          {React.createElement(getPrivacyIcon(cluster.privacy_level), {
                            className: `w-4 h-4 ${getPrivacyColor(cluster.privacy_level)}`
                          })}
                        ...
                        <div className="flex items-center gap-4 text-sm text-gray-400">
                          ...
                          <span>{(cluster.confidence_score * 100).toFixed(1)}% confidence</span>
                        </div>
                      </div>
            ...
            <div className={`${glass.card} p-4`}>
              {selectedCluster ? (
                <div className="space-y-4">
                  <div>
                    <h4 className="text-white font-semibold mb-1">{selectedCluster.cluster_label || 'Unnamed Person'}</h4>
                  ...
                  {!selectedCluster.cluster_label && (
                    <div className="space-y-2">
                      ...
                      />
                      <button
                        onClick={() => labelCluster(selectedCluster.id, newPersonName)}
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-white">{selectedCluster.face_count}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-400">Confidence:</span>
                      <span className="text-white">{(selectedCluster.confidence_score * 100).toFixed(1)}%</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-400">Privacy:</span>
                      <span className={`capitalize ${getPrivacyColor(selectedCluster.privacy_level)}`}>
                        {selectedCluster.privacy_level}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-400">Protected:</span>
                      <span className="text-white">{selectedCluster.is_protected ? 'Yes' : 'No'}</span>
                    </div>
                  </div>
        ...
        {activeTab === 'scan' && (
          ...
          >
            <div className={`${glass.card} p-6`}>
              <h3 className="text-xl font-semibold text-white mb-4">Search by Person</h3>
              ...
              <div className="flex gap-2">
                ...
                />
                <button
                  onClick={() => searchQuery.trim() && searchByPerson(searchQuery)}
                ...
                <div className="mt-4">
                  ...
                  <div className="grid grid-cols-3 md:grid-cols-6 lg:grid-cols-8 gap-2">
                    {searchResults.slice(0, 24).map((photoPath, index) => (
                      <div key={index} className="aspect-square bg-black/20 rounded-lg overflow-hidden">
                        <img
            ...
            <div className={`${glass.card} p-6`}>
              ...
              <p className="text-gray-400 mb-4">
                Scan a directory to detect and cluster faces in your photo collection.
              </p>
        ...
        {activeTab === 'settings' && (
          ...
          >
            <h3 className="text-xl font-semibold text-white">Face Recognition Settings</h3>

            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="text-white font-medium">GPU Acceleration</h4>
                  <p className="text-gray-400 text-sm">Use GPU for faster face processing when available</p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="text-white font-medium">Auto-cluster New Faces</h4>
                  <p className="text-gray-400 text-sm">Automatically group similar faces as they're detected</p>
                </div>
                <button className="w-12 h-6 bg-blue-600 rounded-full relative">
                  <div className="absolute right-1 top-1 w-4 h-4 bg-white rounded-full transition-transform" />
                </button>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="text-white font-medium">Face Quality Threshold</h4>
                  <p className="text-gray-400 text-sm">Minimum quality score for face detection</p>
                </div>
                <input
                  type="range"
                  min="0"
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-gray-300 text-sm">
                    Face recognition data is processed locally on your device and encrypted when stored.
                    You have full control over face data and can delete it at any time.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-gray-400 text-sm">Encrypt all face embeddings for privacy</p>
                </div>
                <button className="w-12 h-6 bg-blue-600 rounded-full relative">
                  <div className="absolute right-1 top-1 w-4 h-4 bg-white rounded-full transition-transform" />
                </button>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-gray-400 text-sm">Hide face detection results until explicitly enabled</p>
                </div>
                <button
                  onClick={() => setPrivacyMode(!privacyMode)}
                  className={`w-12 h-6 ${privacyMode ? 'bg-blue-600' : 'bg-gray-600'} rounded-full relative`}
                >
                  <div className={`absolute ${privacyMode ? 'right-1' : 'left-1'} top-1 w-4 h-4 bg-white rounded-full transition-transform`} />
                </button>
              </div>
