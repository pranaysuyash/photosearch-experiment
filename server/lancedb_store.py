...
class FaceEmbeddingStore(LanceDBStore):
    ...
    def find_similar_faces(
        self, query_embedding: List[float], limit: int = 20, min_confidence: float = 0.0
    ) -> List[Dict]:
        """
    ...
    def get_faces_by_image(self, image_path: str) -> List[Dict]:
        ...
        try:
            # Use SQL-like filter
            results = self.table.search().where(f"image_path = '{image_path}'").to_list()
    ...
    def update_cluster_ids(self, face_cluster_map: Dict[str, int]):
        ...
        # For now, we'll log this operation for monitoring
        logger.info(f"Cluster ID update requested for {len(face_cluster_map)} faces")
        # In a production system, this would be implemented as a batch operation
        # to maintain consistency between the face clustering DB and vector store
