...
class PerformanceTracker:
    ...
    def log_custom_metric(self, operation_name: str, value: float, unit: str = "ms"):
        ...
        self.logger.info(
            f"Custom metric: {operation_name}", extra={"operation": operation_name, "value": value, "unit": unit}
        )
...
def setup_logging(log_level: str = "INFO", log_file: Optional[str] = None) -> tuple:
    ...
    # Create logger
    ps_logger = PhotoSearchLogger(name="PhotoSearch", log_level=log_level, log_file=log_file, enable_console=True)
...
def log_indexing_operation(
    logger: logging.Logger, directory: str, files_processed: int, execution_time: float, errors: int = 0
):
...
def log_error(logger: logging.Logger, error_type: str, error_message: str, context: Optional[Dict[str, Any]] = None):
    ...
    logger.error(f"Error occurred: {error_type}", extra=extra_data)
