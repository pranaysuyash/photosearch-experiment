...
Examples:
  ...
  python format_analyzer.py --summary
    ...
    parser.add_argument("--stats", action="store_true", help="Show format statistics by directory")
    parser.add_argument("--find-format", type=str, help="Find all directories with specific format")
    parser.add_argument("--check-format", type=str, help="Check if format exists in directory")
    parser.add_argument("--list-format", type=str, help="List all files of specific format")
    parser.add_argument("--summary", action="store_true", help="Show format summary across all directories")
    parser.add_argument("--directory", type=str, help="Directory name (partial match supported)")
    parser.add_argument(
    ...
    if not any([args.stats, args.find_format, args.check_format, args.list_format, args.summary]):
    ...
    if args.stats:
        ...
        if exists:
            ...
            catalog_data = catalog.get("catalog", {})
            matching_dirs = [d for d in catalog_data.keys() if args.directory.lower() in d.lower()]
