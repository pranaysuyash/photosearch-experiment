...
class LocationClustersDB:
    ...
    def get_photos_near_location(
        self, latitude: float, longitude: float, radius_km: float = 1.0
    ) -> List[Dict[str, Any]]:
        """Backwards-compatible helper returning dicts.
        ...
        return [
            {
                ...
                "original_place_name": p.original_place_name,
                "corrected_place_name": p.corrected_place_name,
                "country": p.country,
                "region": p.region,
                "city": p.city,
                "accuracy": p.accuracy,
    ...
    def update_photo_location(
        ...
        """Backwards-compatible alias for updating non-coordinate location fields.

        Some older callers/tests expect a method named `update_photo_location`.
        This implementation delegates to `update_place_name`.
        """

        return self.update_place_name(
