...
class OCRSearch:
    ...
    def extract_text_from_image(self, image_path: str, language: str = "eng", config: str = "--psm 6") -> Dict:
        """
        ...
        if not OCR_LIBRARIES_AVAILABLE:
            return {"status": "error", "message": "OCR libraries not available", "text": "", "confidence": 0.0}

        try:
            # Preprocess image
            processed_img = self._preprocess_image(image_path)
            if not processed_img:
                return {"status": "error", "message": "Image preprocessing failed", "text": "", "confidence": 0.0}
            ...
            return {
                ...
                "language": language,
                "statistics": {"word_count": word_count, "char_count": char_count, "line_count": line_count},
            }

        except Exception as e:
            return {"status": "error", "message": str(e), "text": "", "confidence": 0.0}

    def extract_text_from_images(self, image_paths: List[str], language: str = "eng", overwrite: bool = False) -> Dict:
        """
            ...
            try:
                ...
                if result["status"] == "success" and result["text"]:
                    ...
                    cursor.execute(
                        ...
                        VALUES (?, ?, ?, ?)
                    """,
                        (image_path, result["text"], language, result["confidence"]),
                    )
                    ...
                    cursor.execute(
                        ...
                        VALUES (?, ?, ?)
                    """,
                        (image_path, result["text"], language),
                    )
    ...
    def search_text(self, query: str, limit: int = 50, offset: int = 0, language: Optional[str] = None) -> Dict:
        """
        Search for images containing specific text.
    ...
    def get_ocr_stats(self, image_path: str) -> Dict:
        ...
        if not text_row:
            return {"image_path": image_path, "status": "not_processed", "text_content": "", "statistics": {}}
    ...
    def get_ocr_summary(self) -> Dict:
        ...
        for row in cursor.fetchall():
            images_with_most_text.append({"image_path": row["image_path"], "word_count": row["word_count"]})
        ...
        for row in cursor.fetchall():
            recent_ocr.append(
                {"image_path": row["image_path"], "extracted_at": row["extracted_at"], "word_count": row["word_count"]}
            )
